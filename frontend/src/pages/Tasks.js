...
const Tasks = () => {
  ...
  const [profiles, setProfiles] = useState(() => getData(`${formKey}_profiles`) || []);
  const [selectedGroup, setSelectedGroup] = useState(null);
  const [showModal, setShowModal] = useState(false);
  const [editingIndex, setEditingIndex] = useState(null);
  const [method, setMethod] = useState(""); 
  const [twoFaCode, setTwoFaCode] = useState(""); 
  const [showTwoFaInput, setShowTwoFaInput] = useState(false); // Controls 2FA input visibility
  ...
  const handleTaskSubmit = () => {
    if (
      !taskForm.name ||
      taskForm.tickers.some((ticker) => ticker.trim() === "") || // Check for empty tickers
      !taskForm.broker ||
      !taskForm.quantity ||
      !taskForm.action ||
      !taskForm.username ||
    ...
    if (!/^\d+$/.test(taskForm.quantity) || parseInt(taskForm.quantity) <= 0) {
      toast.error('Please enter a valid quantity.');
      return;
    }
    ...
    if (editingIndex !== null) {
      const updatedTasks = [...tasks[selectedGroup]];
      updatedTasks[editingIndex] = { ...taskForm, status: 'Updated' };
      setTasks({ ...tasks, [selectedGroup]: updatedTasks });
      setEditingIndex(null);
    } else {
      setTasks({
        ...tasks,
        [selectedGroup]: [...tasks[selectedGroup], { ...taskForm, status: 'Created' }],
      });
    }
